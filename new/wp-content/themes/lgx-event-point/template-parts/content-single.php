           ...
           <?php echo get_avatar( get_the_author_meta( 'ID' ), 190 ); ?>
            <div class="author-info">
                ...
                <?php 
                    ...
                    $company_url = get_the_author_meta( 'company_url' );
                    $company_name = get_the_author_meta( 'company_name' );
                    $follows_url = get_the_author_meta( 'follows_url' );
                ?>
                <?php echo ( !empty($user_position)? '<h4 class="subtitle">'. get_the_author_meta( 'user_position' ).'</h4>' : '');?>
                <?php echo ( ( !empty($company_url) && !empty($company_name)) ? '<h4 class="comtitle"><a href="'.get_the_author_meta( 'company_url' ).'">'.get_the_author_meta( 'company_name' ).'</a></h4>' : '' );?>
                <?php echo (!empty($follows_url)? '<a class="follow-btn" target="_blank" href="'.get_the_author_meta( 'follows_url' ).'">'.esc_html__('Follow','lgx-event-point').'</a>' : ' ');?>
            </div>
        </div>
        <div class="text-area">
